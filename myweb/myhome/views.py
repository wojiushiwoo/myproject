...
def index(request):
    """
        ...
        [
            {'name':点心/蛋糕','sub':[
                                        {'name':'点心','goodssub':[
                                                                    {goods_objects},{goods_objects},{goods_objects}
                                        ]
                                        },
                                        {'name':'蛋糕','goodssub':[
                                                                    {goods_objects},{goods_objects},{goods_objects}
...
def list(request):
    data = Goods.objects.filter(tyoeid=tid)
...
def search(request):
    # 获取搜索参数
    keyords = request.GET.get('keyords',None)
    if not keyords:
...
def login(request):
    if request.method == 'GET':
        request render(request, 'myhome/login.html')
        ...
        try:
            ...
            # 检测密码是否正确
            res = check_password(request.POST['passord'],password)
            if res:
                # 密码正确
...
def register(request):
    if request.method == 'GET':
        request render(request,'myhome/register.html')
    elif request.method == 'POST':
        # 先验证验证码是否正确
        pass
